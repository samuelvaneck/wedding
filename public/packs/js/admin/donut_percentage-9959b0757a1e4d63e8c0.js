...
var PercentageChart = function PercentageChart(id) {
  var _this = this;
  ...
  this.width = 170;
  this.height = 170;
  this.percentage = $(id).attr("data-percentage");
  this.dataset = {
    lower: calcPercent(0),
  ...
  this.color = d3.scaleOrdinal(["#00539C", "#EEEEEE"]);
  this.arc = d3.arc().innerRadius(radius * .95).outerRadius(radius);
  this.svg = d3.select(id).append("svg").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
  this.path = this.svg.selectAll("path").data(pie(dataset.lower)).enter().append("path").attr("fill", function (d, i) {
    return color(i);
  }).attr("d", arc).each(function (d) {
    _this._current = d;
  });
  this.text = this.svg.append("text").attr("text-anchor", "middle").attr("dy", ".3em");
  this.progress = 0;
};

window.percentageChart = function (id) {
  var _this2 = this;

  setTimeout(function () {
    _this2.path = _this2.path.data(_this2.pie(_this2.dataset.upper));

    _this2.path.transition().duration(_this2.duration).attrTween("d", function (d) {
      var dp = d3.interpolate(_this2._current, d);
      var p = d3.interpolate(_this2.progress, _this2.percentage);
      return function (t) {
        this.text.text(format(p(t) / 100));
        return arc(dp(t));
      };
    });
  }, this.transition);
...
/******/ });
//# sourceMappingURL=donut_percentage-9959b0757a1e4d63e8c0.js.map